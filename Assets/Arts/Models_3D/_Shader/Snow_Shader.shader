Shader "Snow_Shader"
{
	Properties
	{
		...
		_Progression("Progression", Range( 0 , 1.3)) = 0
		_GradientValue("Gradient Value", Range( 0 , 5)) = 2.56729
		_Gradient_A("Gradient_A", Color) = (0.7745483,1,0,0)
		_Gradient_B("Gradient_B", Color) = (0.5283019,0,0,0)
		Snow_Amount("Quantità Neve", Range( 0.001 , 0.5)) = 0.1
	...
	SubShader
	{
		...
		float2 voronoihash211( float2 p )
		{
		...
		float voronoi211( float2 v, inout float2 id )
		{
			float2 n = floor( v );
			float2 f = frac( v );
			float F1 = 8.0;
			...
			for ( int j = -1; j <= 1; j++ )
			{
				for ( int i = -1; i <= 1; i++ )
			 	{
			 		float2 g = float2( i, j );
			 		float2 o = voronoihash211( n + g );
					o = ( sin( -5.8 + o * 6.2831 ) * 0.5 + 0.5 ); float2 r = g - f + o;
					float d = 0.5 * dot( r, r );
			 		if( d<F1 ) {
			 			F2 = F1;
			 			F1 = d; mg = g; mr = r; id = o;
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			float2 coords211 = i.uv_texcoord * 5.42;
			float2 id211 = 0;
			float fade211 = 0.5;
			float voroi211 = 0;
			float rest211 = 0;
			for( int it = 0; it <2; it++ ){
			voroi211 += fade211 * voronoi211( coords211, id211 );
			rest211 += fade211;
			coords211 *= 2;
			fade211 *= 0.5;
			}
			...
			c.a = 1;
			clip( ( Alpha187 * voroi211 * i.uv_texcoord.y ) - Snow_Amount );
			return c;
		}
